...
class FixedDatabaseMigrator {
    ...
    async migrate() {
        ...
        console.log('Camera Vault Database Migration (Fixed)');
        ...
        try {
            ...
            // 3. First, fix the existing cameras table
    ...
    createSafeSlug(brand, model) {
        // Create a URL-safe slug from brand and model
    ...
    async fixExistingCameras() {
        ...
        if (!hasSlug) {
            console.log('  Adding slug column to cameras table...');
            await this.run('ALTER TABLE cameras ADD COLUMN slug TEXT');
        }
        
        // Get all cameras without slugs
        const camerasWithoutSlugs = await this.all('SELECT id, brand, model FROM cameras WHERE slug IS NULL OR slug = ""');
        ...
        if (camerasWithoutSlugs.length > 0) {
            ...
            for (const camera of camerasWithoutSlugs) {
                ...
                let counter = 1;
                while (true) {
                    const existing = await this.get('SELECT id FROM cameras WHERE slug = ? AND id != ?', [finalSlug, camera.id]);
                    if (!existing) break;
        ...
        const expectedColumns = ['full_name', 'release_year', 'sensor_size', 'sensor_type', 
        ...
        for (const colName of expectedColumns) {
            ...
            if (!hasColumn) {
                ...
                let colDef = 'TEXT';
                if (colName.includes('year') || colName.includes('iso') || colName.includes('fps') || colName.includes('usd')) {
                    colDef = 'INTEGER';
                } else if (colName.includes('speed') || colName.includes('megapixels')) {
                    colDef = 'REAL';
                } else if (colName.startsWith('has_') || colName.includes('_sealed')) {
                    colDef = 'BOOLEAN DEFAULT 0';
                } else if (colName.includes('_at')) {
                    colDef = 'DATETIME DEFAULT CURRENT_TIMESTAMP';
                }
                
                try {
                    await this.run(`ALTER TABLE cameras ADD COLUMN ${colName} ${colDef}`);
                } catch (error) {
        ...
        // Update full_name for cameras that don't have it
        await this.run(`UPDATE cameras SET full_name = brand || ' ' || model WHERE full_name IS NULL OR full_name = ''`);
    }
    ...
    async migrateExistingData() {
        ...
        for (const oldCamera of oldCameras) {
            ...
            const slug = oldCamera.slug || this.createSafeSlug(oldCamera.brand, oldCamera.model);
            
            // Map old fields to new comprehensive schema
            const params = [
                // Basic Information
                oldCamera.brand || 'Unknown',
                oldCamera.model || 'Unknown',
                oldCamera.full_name || `${oldCamera.brand || 'Unknown'} ${oldCamera.model || 'Unknown'}`,
                slug,
                oldCamera.category,
